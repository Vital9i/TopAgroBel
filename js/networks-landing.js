    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        redirectToThankYou({
          ...
          source: `Сети и благоустройство · ${String(formData.get("selected_service") || "Заявка")}`,
        });
        return;
...
function validateLeadForm(form) {
  ...
  if (!isCalcOrder) {
    if (form.querySelector('select[name="work_type"][required]')) {
      required.unshift("work_type");
    } else if (form.querySelector('input[name="floor_type"][type="radio"], select[name="floor_type"]')) {
  ...
  required.forEach((name) => {
    ...
    if (name === "work_type") {
      ...
      if (!value) {
        setFieldError(field, "Выберите тип работ");
        ok = false;
      }
      return;
    }
    if (name === "floor_type") {
      ...
      if (radios.length) {
        ...
        if (!checked) {
          ...
          if (err) err.textContent = "Выберите тип работ";
          ok = false;
        }
        return;
      }
...
async function submitLeadForm(formData) {
  ...
  await sendLeadToTelegram({
    ...
    page: "Сети и благоустройство",
    form_label: formData.selected_service || formData.source || "",
    work_type: formData.work_type || formData.floor_type || "",
    scope: formData.scope || formData.floor_area || formData.area || "",
